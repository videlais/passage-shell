...
Passage Shell now includes a web-based runner where you can upload:

- one story HTML file
- one actions JSON file

The web runner executes actions in order, stops on the first failed action, and shows:

- a console/log area with step-by-step progress
- action result payloads
- screenshots generated by `screenshot` actions

Each generated screenshot is shown in the UI with a **Download** link.
...
After building, `npm run start:web` serves the compiled app from `dist/web`.
