...
// Set a default path
// This will be overwritten if not command-line
let filePath = "";

// Test if called on the command-line.
// If so, process.argv will have multiple arguments.
// If not, this was called some other way (probably via an executable)
...
function createBackgroundWindow() {
  ...
  mainWindow.loadFile(filePath + 'twine2/index.html');
...
function createWindow() {
  ...
  // If the settings window is closed, close everything.
  settingsWindow.on('closed', () => {
    app.quit();
  });

  // Check if settings directory exists before loading index.html
  if(fs.existsSync(filePath + 'settings/index.html') ) {

    // Load the settings window
    settingsWindow.loadFile(filePath + 'settings/index.html');
...
ipcMain.on('async-main-file', (event, arg) => {
  settings.file = arg;
});

ipcMain.on('async-main-port', (event, arg) => {
  settings.port = arg;
});
    ...
    if(settings.port != null) {
      ...
      httpServer = webApp.listen(settings.port, () => {
        ...
        if (e.code === 'EADDRINUSE') {
          ...
          console.log("Port number in use!");
          dialog.showErrorBox('Error', 'Port number in use!');
        ...
        portError = true;
        settingsWebContents.send('async-remote-button', true);
      ...
      if(!portError) {
        ...
        try {
          ...
          // Check if mainWindow exists already
          if(mainWindow != null) {
            // Server is restarting, kill window
            mainWindow = null;
          }
          ...
          console.log("Could not create Twine window!");
          dialog.showErrorBox('Error', 'Could not create Twine window!');
